...
/**
 * Class MotorCMSApiPageTest
 */
class MotorCMSApiPageTest extends TestCase
{
    ...
    public function can_show_a_single_page()
    {
        ...
        $this->json('GET',
            '/api/pages/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(200)->seeJson([
            'name' => $record->name
        ]);
    }

    /** @test */
    public function fails_to_show_a_single_page_without_permission()
    {
        $record = create_test_page();
        $this->json('GET',
            '/api/pages/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(403)->seeJson([
            'error' => 'Access denied.'
        ]);
    }
    ...
    public function can_search_for_a_page()
    {
        ...
        $records = create_test_page(10);
        $this->json('GET',
            '/api/pages?api_token=' . $this->user->api_token . '&search=' . $records[2]->name)->seeStatusCode(200)->seeJson([
            'name' => $records[2]->name
        ]);
    }
    ...
    public function can_show_a_second_page_results_page()
    {
        ...
        create_test_page(50);
        $this->json('GET',
            '/api/pages?api_token=' . $this->user->api_token . '&page=2')->seeStatusCode(200)->seeJson([
            'current_page' => 2
        ]);
    }
    ...
    public function fails_if_trying_to_modify_a_page_without_payload()
    {
        ...
        $record = create_test_page();
        $this->json('PATCH',
            '/api/pages/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(422)->seeJson([
            'name' => [ 'The name field is required.' ]
        ]);
    }
